...
const Task = require("../src/models/task.model.ts");
...
const tasks = [
    ...
    },
    {
        ...
        description: "Plan and execute a social media campaign for the new product.",
...
async function seedDB() {
    try {
        ...
        for (let task of createdTasks) {
            await User.findByIdAndUpdate(task.user, { $push: { tasks: task._id } }, { new: true, useFindAndModify: false });
        }
        ...
        mongoose_1.default.connection.close(); // Close the database connection
    }
}
seedDB();
